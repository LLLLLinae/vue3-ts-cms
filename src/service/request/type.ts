...
export interface GFLRequestInterceptors<T = AxiosResponse> {
  interceptorRequestSucc?: (config: AxiosRequestConfig) => AxiosRequestConfig
  interceptorRequestFail?: (error: any) => any
  // 参数的类型可能是AxiosResponse类型，也可能是其他请求定义的泛形
  // 所以不指定具体的类型，通过接口传递的泛形指定
  interceptorResponseSucc?: (res: T) => T
  interceptorResponseFail?: (error: any) => any
}
